...
const DoctorDashboard = () => {
  ...
  const [appointments, setAppointments] = useState([
    { id: 1, patient: "Rahul Gupta", date: "2024-03-30", status: "Pending" },
    { id: 2, patient: "Anish kumar", date: "2024-04-01", status: "Pending" },
    { id: 3, patient: "Rohan kumar", date: "2024-04-01", status: "Pending" },
    { id: 4, patient: "Pilla", date: "2024-04-01", status: "Pending" },
    { id: 5, patient: "Rishab", date: "2024-04-01", status: "Pending" },
  ]);
  ...
  const addDoctor = async () => {
    if (!doctorName || !specialization || !contactNumber)
  ...
  return (
    <div className="bg-gray-900 text-white min-h-screen p-6 md:p-12 mt-[4rem]">
      <ToastContainer position="top-right" autoClose={3000} />
      <h1 className="text-3xl md:text-5xl font-extrabold text-center mb-6 text-blue-400 tracking-wide">
        Doctor Dashboard 🩺
      </h1>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 px-2 md:px-4">
        {/* Add Doctor Section */}
        <div className="bg-gray-800 p-5 md:p-6 rounded-2xl shadow-lg border border-gray-700">
          ...
          <ul className="mt-2 max-h-40 overflow-y-auto border-t border-gray-600 pt-2">
            {doctors.length === 0 ? (
        ...
        <div className="bg-gray-800 p-5 md:p-6 rounded-2xl shadow-lg border border-gray-700">
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-4">Manage Appointments</h2>
          {appointments.length === 0 ? (
            <p className="text-gray-400">No pending appointments.</p>
          ) : (
            <ul className="max-h-40 overflow-y-auto border-t border-gray-600 pt-2">
              {appointments.map((appointment) => (
                <li
                  key={appointment.id}
                  className="flex flex-col md:flex-row justify-between items-center border-b border-gray-600 py-3"
                >
                  <span className="text-lg text-gray-300 text-center md:text-left">
